                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    ...
                                    <div class="example-box-wrapper">
                                        <form method="post" action="<?= base_url() ?>production/edit" id="SubmitAdd" class="form-horizontal bordered-row" onsubmit="return validation_for_raw_metrial();">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <select onchange="GetProductsWithSection(this.value, 'append_section')" name="section" id="section" class="chosen-select">
...
//                                                        print_r($inv_item->qty);
                                                        $html .= '<div class="col-sm-12 prod" id="prod' . $id . '"><div class="col-sm-6">';
...
//                echo $key . "<br>";
                                                            foreach ($product_suggestions[$key] as $value) {
                                                                if ($pro_item->product_id == $value->product_id) {
                                                                    $unit = $value->unit_symbol;
                                                                }
                                                                $html .= "<option " . ($pro_item->product_id == $value->product_id ? "selected" : "") . " value='" . $value->product_id . "'>" . $value->product_name . "</option>";
                                                            }
                                                            $html .= "</optgroup>";
                                                        }
                                                        $html .= '</select></div>';
                                                        $html .= '</div><div class="col-sm-5"><div id="div_qty' . $id . '" class="input-group"><input onkeydown="checkTabPress(event);" type="number" name="qty[]" step=".00001"  value="' . $pro_item->credit_qty . '" id="qty' . $id . '" class="form-control" placeholder="Qty"><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
...
//                                                        print_r($inv_item->qty);
                                                        $html .= '<div class="col-sm-12 prod_raw" id="raw' . $raw_id . '"><div class="col-sm-3">';
                                                        $html .= '<div class="input-group"><select onchange=' . 'add_units_raw(this.id,this.options[this.selectedIndex].getAttribute("unit_symbol"));GetTotalIssue(this.value,this.id);' . '  name="product_name_raw[]" id="product_name_raw' . $raw_id . '" class="chosen-select">';
                                                        $html .= "<option value=''>Select or Type Product</option>";
                                                        foreach ($product_suggestions_raw as $key => $value) {
                                                            $html .= "<optgroup label='" . $key . "'>";
//                echo $key . "<br>";
                                                            foreach ($product_suggestions_raw[$key] as $value) {
                                                                if ($raw_item->product_id == $value->product_id) {
                                                                    $unit_raw = $value->unit_symbol;
                                                                }
                                                                $html .= "<option " . ($raw_item->product_id == $value->product_id ? "selected" : "") . " value='" . $value->product_id . "' unit_symbol='" . $value->unit_symbol . "'>" . $value->product_name . "</option>";
                                                            }
                                                            $html .= "</optgroup>";
                                                        }
                                                        $html .= '</select></div>';
                                                        $html .= '</div><div class="col-sm-3"><div id="div_qty_raw' . $raw_id . '" class="input-group"><input onkeyup="calculate_issue(this.value,this.id);" onkeydown="checkTabPress(event);" type="number" name="qty_raw[]" step=".00001" value="' . $raw_item->debit_qty . '" id="qty_raw' . $raw_id . '" class="form-control" placeholder="Qty"><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit_raw . '</span></div></div>';
                                                        foreach ($raw_items_old as $old_it) {
                                                            if ($old_it->product_id == $raw_item->product_id) {
                                                                $html .= '<div class="col-sm-2"><div id="div_issue' . $raw_id . '" class="input-group"><input type="number"  onkeyup="checkMax(this.value,this.id);" name="issue[]" id="issue' . $raw_id . '" class="form-control" placeholder="Total Issue Quantity" value="' . $old_it->old_tot_issue . '" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit_raw . '</span></div></div>';
                                                                $html .= '<div class="col-sm-2"><div id="div_remaining_issue' . $raw_id . '" class="input-group"><input type="number"  onkeyup="checkMax(this.value,this.id);" name="remaining_issue[]" id="remaining_issue' . $raw_id . '" class="form-control" placeholder="Remaining Issue Quantity" value="' . sprintf("%.2f", ($old_it->old_tot_issue - $raw_item->debit_qty)) . '" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit_raw . '</span></div></div>';
                                                            }
                                                        }
                                                        $html .= '<input type="hidden" id="mode_hidden' . $raw_id . '" name=mode_hidden[] value="no"/><div class="col-sm-1"><div id="div_mode' . $raw_id . '" class="input-group"><select disabled onchange="set_mode_hidden(this.value,this.id);"  name="mode[]" id="mode' . $raw_id . '" class="form-control"><option value="no"> Select Mode</option>';

                                                        $m_opt = '';
                                                        foreach ($modes as $m) {
                                                            if ($raw_item->description == $m->loss_gain_mode.'-') {
                                                                $m_opt .= '<option selected value="' . $m->loss_gain_mode . '">' . $m->loss_gain_mode . '</option>';
                                                            } else {
                                                        ...
                                                        $html .= '</select></div></div>';
                                                        $html .= '<div class="col-sm-1"><div class="input-group"><button type="button"  onclick="DelRaw(' . $raw_id . ')" class="btn btn-danger cross" onkeydown="checkTabPress(event);">X</button></div></div> </div>';
...
foreach ($raw_items as $raw_item) {
    foreach ($modes as $m) {
        if ($raw_item->description == $m->loss_gain_mode.'-') {
            ?>
                                                                set_mode_hidden("<?= $m->loss_gain_mode ?>", "mode<?= $raw_id ?>",true);
...
//                                                document.getElementsByClassName("discount").addEventListener('keyup', checkTabPress);
                                                ...
                                                function add_product() {
                                                    ...
                                                    html += '</div><div class="col-sm-5"><div id="div_qty' + id + '" class="input-group"><input type="number" onkeydown="checkTabPress(event);" name="qty[]" step=".00001" id="qty' + id + '" class="form-control" placeholder="Qty"></div></div>';
                                                    html += '<div class="col-sm-1"><div class="input-group"><button type="button"  onclick="DelProd(' + id + ')" class="btn btn-danger cross" onkeydown="checkTabPress(event);">X</button></div></div> </div>';
//                                                    alert(html);
...
//                                                    alert($("#product_name" + id).next("div").children("a").text());
                                                ...
                                                function add_raw() {
                                                    ...
                                                    html += '</div><div class="col-sm-3"><div id="div_qty_raw' + id + '" class="input-group"><input onkeyup="calculate_issue(this.value,this.id);" type="number" onkeydown="checkTabPress(event);" name="qty_raw[]" step=".00001" id="qty_raw' + id + '" class="form-control" placeholder="Qty"></div></div>';
                                                    html += '<div class="col-sm-2"><div id="div_qty_raw' + id + '" class="input-group"><input type="number"  onkeyup="checkMax(this.value,this.id);" name="issue[]" id="issue' + id + '" class="form-control" placeholder="Total Issue Quantity" readonly></div></div>';
                                                    html += '<div class="col-sm-2"><div id="div_qty_raw' + id + '" class="input-group"><input type="number"  onkeyup="checkMax(this.value,this.id);" name="remaining_issue[]" id="remaining_issue' + id + '" class="form-control" placeholder="Remaining Issue Quantity" readonly></div></div>';
...
//                                                    alert($("#product_name" + id).next("div").children("a").text());
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-12">
                                                    <textarea name="desc" id="desc"> <?= $issue[0]->description ?></textarea>
                                                </div>
                                            </div>
                                            <div class="form-group">
...
//                        alert("no here");
                ...
                function DelRaw(myid_prod) {
                    var myid_prod_new = "raw" + myid_prod;
                    $("#" + myid_prod_new).remove();
                    CalculateSubTotal(myid_prod);
                }
...
//
                ...
                function calculate_issue(val, id) {
                    ...
                    if (val == "") {
                        ...
                        if ($("#remaining_issue" + id).val() < 0) {
                            ...
                            $("#mode" + id).prop('disabled', true);
                            $("#mode" + id).removeClass("ch_mode");
                            $("#mode_hidden" + id).val("no");
                        }
                    }
                ...
                function set_mode_hidden(val, id,flag=FALSE) {
                    id = id.replace("mode", "");
