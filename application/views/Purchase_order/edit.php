...
<!--<link rel="stylesheet" type="text/css" href="<?= base_url() ?>assets/widgets/chosen/chosen.css">-->
            ...
            // });
                            ...
                            <div id="page-title">
                                ...
                                <p>Edit Required information for purchase Order</p>
                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    ...
                                    <div class="example-box-wrapper">
                                        <form method="post" action="<?= base_url() ?>purchase_order/edit" id="SubmitAdd" class="form-horizontal bordered-row">
...
//                                                    print_r($ordr_items);
//                                                    die();
                                                    foreach ($ordr_items as $ordr_item) {

//                                                        print_r($ordr_item->qty);
...
//                echo $key . "<br>";
                                                            foreach ($product_suggestions[$key] as $value) {
                                                                if ($ordr_item->product_id == $value->product_id) {
                                                                    $unit = $value->unit_symbol;
                                                                }
                                                                $html .= "<option " . ($ordr_item->product_id == $value->product_id ? "selected" : "") . " value='" . $value->product_id . "' unit_symbol='" . $value->unit_symbol . "'>" . $value->product_name . "</option>";
                                                            }
                                                            $html .= "</optgroup>";
                                                        }
                                                        $html .= '</select></div>';
                                                        $html .= '</div><div class="col-sm-5"><div id="div_qty' . $id . '" class="input-group"><input type="number" name="qty[]" step=".01" step=".01" value="' . $ordr_item->qty . '" id="qty' . $id . '" class="form-control" placeholder="Qty"><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2" style="display:none;"><div id="div_purchase_price' . $id . '" class="input-group"><input type="number" onblur="setTwoNumberDecimal(this.id)" onkeyup="CalculateSubTotal(' . $id . ');" step="0.01" name="purchase_price[]" id="purchase_price' . $id . '" value="' . $ordr_item->product_purchase_price . '" class="form-control" placeholder="purchase Price"><span class="input-group-addon bootstrap-touchspin-postfix">Rs</span></div></div>';
                                                        $html .= '<div class="col-sm-2" style="width:13%; display:none;"><div id="div_discount' . $id . '" class="input-group"><input type="number" onblur="setTwoNumberDecimal(this.id)" step="0.01" onkeydown="checkTabPress(event);" onkeyup="CalculateSubTotal(' . $id . ');"  name="discount[]" id="discount' . $id . '" value="' . $ordr_item->discount . '" class="form-control discount" placeholder="Discount"><span class="input-group-addon bootstrap-touchspin-postfix">Rs</span></div></div>';
                                                        $html .= '<div class="col-sm-3" style="width:12%; display:none;"><div id="div_batch' . $id . '" class="input-group"><select onchange="GetBatchProCount(this.value,' . $id . ')" name="batch[]" id="batch' . $id . '" class="chosen-select">';
                                                        $html .= '<option value="">Batch No</option>';
                                                        foreach ($batches as $batch) {
                                                            $html .= '<option ' . ($ordr_item->batch == $batch->batch_no ? 'selected' : '') . ' value="' . $batch->batch_no . '">' . $batch->batch_no . '</option>';
                                                        }
                                                        $html .= '</select></div></div>';
                                                        $html .= '<div class="col-sm-1" style="width:12%;  display:none;"><div class="input-group"><input type="number" readonly="" onblur="setTwoNumberDecimal(this.id)" step="0.01" name="sub_total[]" id="sub_total' . $id . '" class="form-control sub_total" value="' . $ordr_item->ordr_subtotal . '" placeholder="Sub Total"></div></div>';
                                                ...
                                                <div class="col-sm-12">
                                                    ...
                                                    <div id="total" class="col-sm-2" style="font-size: 22px; font-weight: bold; color:#003eff;"><span><?= $ordr[0]->ordr_total ?></span> Rs
                                                        <input type="hidden" name="ordr_total" id="ordr_total" value="<?= $ordr[0]->ordr_total ?>" />
                                                    </div>
                                                </div>
...
//                                                document.getElementsByClassName("discount").addEventListener('keyup', checkTabPress);
                                                ...
                                                function add_product() {
                                                    ...
                                                    html += '</div><div class="col-sm-5"><div id="div_qty' + id + '" class="input-group"><input type="number" name="qty[]" step=".01" step=".01" id="qty' + id + '" class="form-control" placeholder="Qty"></div></div>';
                                                    html += '<div class="col-sm-2" style="display:none;"><div id="div_purchase_price' + id + '" class="input-group"><input type="number" onblur="setTwoNumberDecimal(this.id)" onkeyup="CalculateSubTotal(' + id + ');" step="0.01" name="purchase_price[]" id="purchase_price' + id + '" class="form-control" placeholder="purchase Price"></div></div>';
                                                    html += '<div class="col-sm-2" style="width:13%; display:none;"><div id="div_discount' + id + '" class="input-group"><input type="number" onblur="setTwoNumberDecimal(this.id)" step="0.01" onkeydown="checkTabPress(event);" onkeyup="CalculateSubTotal(' + id + ');"  name="discount[]" id="discount' + id + '" class="form-control discount" placeholder="Discount"></div></div>';
                                                    html += '<div class="col-sm-3" style="width:12%; display:none;"><div id="div_batch' + id + '" class="input-group"><select onchange="GetBatchProCount(this.value,' + id + ')" name="batch[]" id="batch' + id + '" class="chosen-select">' + $("#batch_suggestions").val() + '</select></div></div>';
                                                    html += '<div class="col-sm-1" style="width:12%; display:none;"><div class="input-group"><input type="number" readonly="" onblur="setTwoNumberDecimal(this.id)" step="0.01" name="sub_total[]" id="sub_total' + id + '" value="0.00" class="form-control sub_total" placeholder="Sub Total"></div></div>';
                                                    html += '<div class="col-sm-1" style="width:4%;"><div class="input-group"><button type="button"  onclick="DelProd(' + id + ')" class="btn btn-danger cross" onkeydown="checkTabPress(event);">X</button></div></div> </div>';
//                                                    alert(html);
...
//                                                    alert($("#product_name" + id).next("div").children("a").text());
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-2">
                                                    <input  type="text" name="sv_no" id="sv_no" class="form-control" readonly="" value="<?= $ordr[0]->voucher_no ?>">
                                                </div>
                                            ...
                                            <div class="form-group" style="display: none;">
                                                ...
                                                <div class="col-sm-2">
                                                    <input  type="text" name="cargo" id="cargo" class="form-control" placeholder="Enter Batch No" value="<?= $ordr[0]->cargo ?>">
                                                </div>

                                                <label class="col-sm-2 control-label">Bilty No</label>
                                                <div class="col-sm-2">
                                                    <input  type="text" name="builty_no" id="builty_no" class="form-control" placeholder="Enter Batch No" value="<?= $ordr[0]->builty_no ?>">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <button id="confrm_btn" class="btn btn-alt btn-hover btn-blue-alt col-md-4" onclick="return ConfirmAdd();">
                                                    <span>CONFIRM EDIT purchase</span>
                                                    <i class="glyph-icon icon-arrow-right"></i>
                                                </button>
                                                <div class="col-md-4 col-sm-0"></div>
                                            </div>
