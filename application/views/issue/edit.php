...
<!--<link rel="stylesheet" type="text/css" href="<?= base_url() ?>assets/widgets/chosen/chosen.css">-->
            ...
            // });
                            ...
                            <div id="page-title">
                                <h2>Edit Issue</h2>
                                <p>Required information for Issue</p>
                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    ...
                                    <div class="example-box-wrapper">
                                        <form method="post" action="<?= base_url() ?>issue/edit" id="SubmitAdd" class="form-horizontal bordered-row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-2">
                                                    <select name="warehouse" id="warehouse" class="chosen-select" onchange="GetProductsByWarehouse(this.value, 'append_warehouse');">
...
//                                                        print_r($inv_item->qty);
                                                        ...
                                                        $html .= '<div class="input-group"><select onchange=' . 'add_units(this.id,this.options[this.selectedIndex].getAttribute("unit_symbol"));GetInstock(this.value,this.id)' . '   name="product_name[]" id="product_name' . $id . '" class="chosen-select">';
...
//                echo $key . "<br>";
                                                            foreach ($product_suggestions[$key] as $value) {
                                                                ...
                                                                $html .= "<option " . ($issue_item->product_id == $value->product_id ? "selected" : "") . " value='" . $value->product_id . "' unit_symbol='" . $value->unit_symbol . "'>" . $value->product_name . "</option>";
                                                            }
                                                            $html .= "</optgroup>";
                                                        }
                                                        $html .= '</select></div>';
                                                        $html .= '</div><div class="col-sm-3"><div id="div_qty' . $id . '" class="input-group"><input onkeydown="checkTabPress(event);" type="number" step=".01"  name="qty[]"  step=".01" onkeyup="Calculate_instock(this.value,this.id);" value="' . $issue_item->credit_qty . '" id="qty' . $id . '" class="form-control qty" placeholder="Qty"><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2"><div id="div_instock' . $id . '" class="input-group"><input type="number" step=".01"  name="instock[]" id="instock' . $id . '" class="form-control" placeholder="Total Instock" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2"><div id="div_remaining_instock' . $id . '" class="input-group"><input type="number" step=".01"  name="remaining_instock[]" id="remaining_instock' . $id . '" class="form-control" placeholder="Remaining Instock" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
...
//                    alert(data);
                ...
                function GetInstock(product_id, product_name_id, flag = false) {
                    ...
                    var base_url = "<?= base_url() ?>";
                    $.post(base_url + "Issue/GetInstockWithDate", {product_id: product_id, datee: datee})
                            .done(function (data) {
                                var obj = JSON.parse(data);

                                $("#instock" + product_name_id).val(obj[0].product_balnc);
                                $("#remaining_instock" + product_name_id).val(obj[0].product_balnc);
                                if (flag == true) {
                                    var qt = $('#qty' + product_name_id).val();
                                    $("#remaining_instock" + product_name_id).val(parseFloat(obj[0].product_balnc) - parseFloat(qt));
                                }
