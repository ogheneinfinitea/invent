...
//                $('.dataTables_filter input').attr("placeholder", "Search...");
                            ...
                            <div id="page-title" style="overflow: hidden;">
                                <div class="col-md-8">
                                    <h2>Issue Slips</h2>
                                    <p>Add, Edit or Delete Issue Slips</p>
                                </div>
                                ...
                                <div class="col-md-2">
                                    <button class="btn btn-blue-alt pull-right" onclick="AddIssue();"><span style="margin-right: 5px;" class="glyph-icon icon-plus"></span>Issue New Slip</button>
                                </div>
                            </div>
                            <div id="collapseOne" class="panel-collapse collapse" aria-expanded="false">
                                <div class="panel-body">
                                    <div class="row" class="select_date">
                                        <form name="AccountForm" method="post" id="AccountForm" action="<?= base_url() ?>issue/" class="form-horizontal">
                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    <div class="row ">
                                        <a href="<?= base_url() ?>Issue"><h3 class="title-hero col-md-9">
                                                All Issue Slips
                                            </h3></a>
                                    ...
                                    <table class=" table table-striped table-bordered responsive no-wrap dataTable no-footer dtr-inline general">
                                        <tbody class="post-list" id="postList">
                                            ...
                                            <?php
                                            if (!empty($issues)): $count = 1;
                                                foreach ($issues as $issue):
                                                    ?>
                                                    <tr <?= $issue['return_issue_id'] != NULL ? "style='background:#ffe5e5 !important;'" : "" ?>>
                                                        ...
                                                        <td >
                                                            <?php if ($this->session->userdata('user_group_id') == 1 && ($issue['return_issue_id'] == NULL)) { ?>
                                                                <button class="btn btn-round btn-info" data-toggle="tooltip" data-placement="top" title="Edit" data-toggle="modal" data-target="#myModal" onclick="EditIssue('<?= $issue['issue_id'] ?>');">
                                                                    <i class="glyph-icon icon-pencil"></i>
                                                                </button>
                                                                <button class="btn btn-round btn-danger" data-toggle="tooltip" data-placement="top" title="Delete" onclick="DeleteIssue('<?= $issue['issue_id'] ?>');">
                                                                    <i class="glyph-icon icon-trash"></i>
                                                                </button>
                                                            <?php } ?>
                                                            <button class="btn btn-round btn-success" data-toggle="tooltip" data-placement="top" title="View" onclick="ViewIssue('<?= $issue['issue_id'] ?>');">
                                                                <i class="glyph-icon icon-file-text-o"></i>
                                                            </button>
                                                            <button class="btn btn-round btn-black" data-toggle="tooltip" data-placement="top" title="Return" data-toggle="modal" data-target="#myModal" onclick="IssueReturn('<?= $issue['issue_id'] ?>');">
                                                                <i class="glyph-icon icon-refresh"></i>
                                                            </button>
                                                        </td>
                                                    </tr>
                                        ...
                                        if (isset($date_from) && isset($date_to) && isset($pro)) {
                                            if (empty($pro)) {
                                                $pro = NULL;
                                            }
            ...
            <script>
                                                        //                                                            var start = $("#start").val();
                                                        //
                                                        //                                                            $.post("<?= base_url() ?>Issue", {start: start})
                                                        //                                                                    .done(function (data) {
            ...
            <script>
                function EditIssue(issue_id) {
                    window.location.href = "<?= base_url() ?>issue/view/" + issue_id;
                ...
                function DeleteIssue(issue_id) {
                    ...
                    if (cnfirm) {
                        $.post("<?= base_url() ?>issue/delete", {id: issue_id})
                                .done(function (data) {
                                    //                                                                                    alert(data);
                                    if ($.trim(data) == 'done') {
                ...
                function IssueReturn(issue_id) {
                    window.location.href = "<?= base_url() ?>issue/view_return/" + issue_id;
                }

                function AddIssue() {
                    window.location.href = "<?= base_url() ?>issue/add";
                }
                function ViewIssue(issue_id) {
                    window.location.href = "<?= base_url() ?>issue/view_slip/" + issue_id;
                }
                ...
                $("#search").keyup(function (event) {
                    ...
                    if (keycode == '13') {
                        ...
                        if (rtngPtId == "") {
                            ...
                            $.post("<?= base_url() ?>Issue/search", {id: rtngPtId})
                                    .done(function (data) {
