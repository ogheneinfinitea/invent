                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    ...
                                    <div class="example-box-wrapper">
                                        <form method="post" action="<?= base_url() ?>received/edit" id="SubmitAdd" class="form-horizontal bordered-row">
                                            <input type="hidden" name="issue_id" value="<?= $issue[0]->issue_id ?>"/>
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-2">
                                                    <div class="input-prepend input-group">
                                                        ...
                                                        <input readonly id="date" name="date" type="text" class="bootstrap-datepicker form-control" value="<?php
                                                        $date = explode(" ", $issue[0]->date);
                                                ...
                                                <div class="col-sm-2">
                                                    <select readonly="readonly"  name="warehouse" id="warehouse" class="chosen-select" >
                                                        <option value="">Select Option</option>
                                                        <?php foreach ($warehouses as $warehouse) { ?>
                                                            <option <?= $issue[0]->warehouse_id == $warehouse->warehouse_id ? "selected" : "" ?> value="<?= $warehouse->warehouse_id ?>"><?= $warehouse->warehouse_name ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <label class="col-sm-2 control-label">Select Section</label>
                                                <div class="col-sm-2">
                                                    <select readonly="readonly" name="section" id="section" class="chosen-select">
                                                        <option value="">Select Option</option>
                                                        <?php foreach ($sections as $section) { ?>
                                                            <option <?= $issue[0]->section_id == $section->section_id ? "selected" : "" ?> value="<?= $section->section_id ?>"><?= $section->section_name ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <button style="display:none;"  type="button" onclick="add_product();" class="btn btn-danger pull-right" style="height: 20px;padding: 0;padding-right: 2px;padding-left: 2px;line-height: 20px;">
...
//                                                        print_r($inv_item->qty);
                                                        $html .= '<div class="col-sm-12 prod" id="prod' . $id . '"><div class="col-sm-2">';
                                                        $html .= '<div class="input-group"><select readonly="readonly"    name="product_name[]" id="product_name' . $id . '" class="chosen-select pro">';
...
//                echo $key . "<br>";
                                                            foreach ($product_suggestions[$key] as $value) {
                                                                if ($issue_item->product_id == $value->product_id) {
                                                                    $unit = $value->unit_symbol;
                                                                }
                                                                $html .= "<option " . ($issue_item->product_id == $value->product_id ? "selected" : "") . " value='" . $value->product_id . "'>" . $value->product_name . "</option>";
                                                            }
                                                            $html .= "</optgroup>";
                                                        }
                                                        $html .= '</select></div>';
                                                        $html .= '</div><div class="col-sm-2"><div class="input-group"><input onkeydown="checkTabPress(event);" type="text"  name="qty[]" step=".01" onkeyup="Calculate_instock(this.value,this.id);" value="' . $issue_item->credit_qty . '" id="qty' . $id . '" class="form-control qty" placeholder="Qty" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2"><div class="input-group"><input onkeyup="Calculate_remaining_qty(this.value,this.id);" type="text"  name="lost_qty[]" step=".01" id="lost_qty' . $id . '" class="form-control" placeholder="Lost" ><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2"><div class="input-group"><input type="text"  name="remaining_qty[]" step=".01" id="remaining_qty' . $id . '" class="form-control" placeholder="Remaining Quantity" ><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';

                                                        $html .= '<div class="col-sm-2"><div class="input-group"><input type="text"  name="instock[]" id="instock' . $id . '" class="form-control" placeholder="Total Instock" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div class="col-sm-2"><div class="input-group"><input type="text"  name="remaining_instock[]" id="remaining_instock' . $id . '" class="form-control" placeholder="Remaining Instock" readonly><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<div style="margin-top: 10px;margin-bottom: 10px;" class="col-md-6 pull-right"><div class="input-group"><input type="text" onkeyup="Calculate_remaining_qty_notes(this.value,this.id);"  name="lost_notes[]" id="lost_notes' . $id . '" class="form-control" placeholder="Lost Notes" ><span class="input-group-addon bootstrap-touchspin-postfix">' . $unit . '</span></div></div>';
                                                        $html .= '<input type=hidden id="all_rec' . $id . '" value="' . $issue_item->product_id . '*0*' . $issue_item->credit_qty . '*nil' . '" name="all_rec[]"/></div>';
                                                        $id++;
                                                    }
                                                    echo $html;
                                                    ?>
...
foreach ($issue_items as $issue_item) {
        ...
        foreach ($product_suggestions[$key] as $value) {
            if ($issue_item->product_id == $value->product_id) {
                ?>
                                                                    ...
                                                                    var product_name_id = "product_name<?= $id ?>";
                                                                    GetInstock(product_id, product_name_id, true);
    ...
    ?>
                                                        var select_pro<?= $id ?> = $('#product_name<?= $id ?>');
                                                        var oldVal_pro<?= $id ?> = $("#product_name<?= $id ?>").val();
                                                        select_pro<?= $id ?>.chosen().on('change', function (e) {
                                                            select_pro<?= $id ?>.val(oldVal_pro<?= $id ?>);
                                                            select_pro<?= $id ?>.trigger("chosen:updated");
                                                        });
...
?>
                                                ...
                                                function add_product() {
                                                    ...
                                                    html += '<div class="input-group"><select onchange="GetInstock(this.value,this.id);"  name="product_name[]" id="product_name' + id + '" class="chosen-select">' + append_warehouse + '</select></div>';
                                                    html += '</div><div class="col-sm-3"><div class="input-group"><input type="number" onkeydown="checkTabPress(event);" onkeyup="Calculate_instock(this.value,this.id);" name="qty[]" step=".01" id="qty' + id + '" class="form-control" placeholder="Qty"></div></div>';
                                                    html += '<div class="col-sm-2"><div class="input-group"><input type="number"  name="instock[]" step=".01" id="instock' + id + '" class="form-control" placeholder="Total Instock" readonly></div></div>';
                                                    html += '<div class="col-sm-2"><div class="input-group"><input type="number"  name="remaining_instock[]" step=".01" id="remaining_instock' + id + '" class="form-control" placeholder="Remaining Instock" readonly></div></div>';
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-12">
                                                    <textarea readonly="" name="desc" id="desc"> <?= $issue[0]->description ?></textarea>
                                                </div>
                                            </div>
                                            <div class="form-group">
...
//                    alert(data);
                ...
                function GetInstock(product_id, product_name_id, flag = false) {
...
//                                console.log(obj.instock);
                                ...
                                if (flag == true) {
                                    var qt = $('#qty' + product_name_id).val();
                                    $("#remaining_instock" + product_name_id).val(parseFloat(obj.instock) - parseFloat(qt));
                                }
...
//                        console.log(updated_remaining);
                ...
                function Calculate_remaining_qty(val, id) {

                    id = id.replace("lost_qty", "");
                    var instock = $("#qty" + id).val();
                    if (val == "") {

                        $("#remaining_qty" + id).val(instock);
                    } else {

                        var updated_remaining = instock - val;
//                        console.log(updated_remaining);
                        $("#remaining_qty" + id).val(parseFloat(updated_remaining).toFixed(2));
                    }

                    var all_rec = $("#all_rec" + id).val();
                    var remaining_qty = $("#remaining_qty" + id).val();
                    var lost_notes = $("#lost_notes" + id).val();
                    var result = all_rec.split('*');
                    result[1] = val;
                    if (remaining_qty != "") {
                        result[2] = remaining_qty;
                    }
                    if (lost_notes != "") {
                        result[3] = lost_notes;
                    }
                    $("#all_rec" + id).val(result[0] + "*" + result[1] + "*" + result[2] + "*" + result[3]);

                }
                function Calculate_remaining_qty_notes(val, id) {
                    id = id.replace("lost_notes", "");
                    var all_rec = $("#all_rec" + id).val();
                    var result = all_rec.split('*');
                    result[3] = val;
