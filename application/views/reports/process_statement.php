...
<html  lang="en">
    ...
    <head>
        ...
        <!--[if IE]><meta http-equiv='X-UA-Compatible' content='IE=edge,chrome=1'><![endif]-->
            <title> Facteezo: Process Statement </title>
        ...
        <script>
            ...
            function printProductLedger(divName) {
...
<style>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <form name="ProductForm" method="post" id="ProductForm" action="<?= base_url() ?>reports/process_statement" class="form-horizontal">
    ...
    <!--<link rel="stylesheet" type="text/css" href="<?= base_url() ?>assets/widgets/datatable/datatable.css">-->
                            ...
                            <div id="page-title" style="overflow: hidden;">
                                <h2>Process & Issue Report<?= !empty($account_name) ? " for <i>" . $account_name . "</i>" : "" ?></h2>
                                <p>Detailed report<?= isset($from_date) ? " from <b>" . date("j F, Y", strtotime($from_date)) . "</b> To <b>" . date("j F, Y", strtotime($to_date)) . "</b>" : "" ?></p>

                                <button class="btn btn-primary btn-print" onclick="printProductLedger('PrintArea');">Print</button>
                                            ...
                                            <tbody>
                                                ...
                                                if (!empty($prev_balnc)) {
                                                    foreach ($prev_balnc as $balnc) {
                                                        ...
                                                        echo "<td></td>";
                                                        echo "<td></td>";
                                                        echo "<td>" . $balnc->product_balnc . ' ' . $balnc->unit_symbol . "</td>";
                                                        echo "</tr>";
...
//                                                    }
                                                            ...
                                                            foreach ($products_report as $value) {
                                                                if ($balnc->product_id == $value->product_id) {
                                                                    $value->debit_qty == null ? $value->debit_qty = 0 : $value->debit_qty = $value->debit_qty;
                                                                    $value->credit_qty == null ? $value->credit_qty = 0 : $value->credit_qty = $value->credit_qty;
                                                                    $balance = ($balance + $value->credit_qty) - $value->debit_qty;
                                                                    ?>
                                                                    <tr>
                                                                        <td><?= date("d/m/Y", strtotime($value->date_ledger)) ?></td>
                                                                        <td><?= $value->product_name ?></td>
                                                                        <td><?= ($value->product_ledger_description=="adjustment"?"Adjustment<br>":"").html_entity_decode($value->product_ledger_description) ?></td>
                                                                        <td><?= $value->debit_qty . ' ' . $value->unit_symbol ?></td>
                                                                        <td><?= $value->credit_qty . ' ' . $value->unit_symbol ?></td>
                                                                        <td><?= number_format($balance, 2, '.', '') . ' ' . $value->unit_symbol ?></td>
                                                                    </tr>
                                                                    <?php
                                                                    $total_debit = ($total_debit + $value->debit_qty);
                                                                    $total_credit = ($total_credit + $value->credit_qty);
                                                ...
                                                ?>
                                                <tr>
                                                    <td colspan="3" style="font-size: 18px; font-style: italic"><b>Total Calculated</b></td>
                                                    <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_debit, 2, '.', '') . ' ' . $unit ?> </b></td>
                                                    <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_credit, 2, '.', '') . ' ' . $unit ?> </b></td>
                                                    <td style="font-size: 18px; font-style: italic"><b><?= number_format(($balance), 2, '.', '') . ' ' . $unit ?> </b></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
