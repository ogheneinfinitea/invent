...
<html  lang="en">
    <head>
        ...
        <title> Facteezo:Bank Statement </title>
        ...
        <script>
            ...
            //  function printAccountLedgerProduct(divName) {
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <form name="AccountForm" method="post" id="AccountForm" action="<?= base_url() ?>reports/purchase_closing_statement" class="form-horizontal">
                                <div class="form-group">
                                    <label class="col-sm-3 control-label">Select Account </label>
                                    <div class="col-sm-5" id="account_div">
                                        <select name="account" id="account" class="chosen-select">
                                            <option id="option_after" value="">ALL Purchase ACCOUNTS</option>
    ...
    <!--<link rel="stylesheet" type="text/css" href="<?= base_url() ?>assets/widgets/datatable/datatable.css">-->
                            ...
                            <div id="page-title" style="overflow: hidden;">
                                <h2>Purchase Closing Statement Report<!-- <?= !empty($account_name) ? " for <i>" . $account_name . "</i>" : "" ?> --></h2>
                                <p>Detailed report<?= isset($from_date) ? " from <b>" . date("j F, Y", strtotime($from_date)) . "</b> To <b>" . date("j F, Y", strtotime($to_date)) . "</b>" : "" ?></p>

                                <button class="btn btn-primary btn-print" onclick="printReport('PrintArea');">Print Detail Summary</button>
                                            ...
                                            <tbody>
                                                ...
                                                if ($all_accounts == 'no') {
                                                    ...
                                                    if (!empty($account_report)) {
                                                        $ledger_count = 0;
                                                        foreach ($account_report as $value) {
                                                            $balance = ($balance + $value->debit_amount ) - $value->credit_amount;

                                                            $description = explode("&lt;/p&gt;&lt;p&gt;", $value->description);
                                                            ?>
                                                            <tr>
                                                                <?php
                                                                $total_debit = $total_debit + ($value->debit_amount );

                                                                $total_credit = $total_credit + ($value->credit_amount);
                                                                $ledger_count++;
                                                            }
                                                        }
                                                        ?>
                                                    <tr>
                                                        <td>1</td>
                                                        <td><?= $account_name ?></td>
                                                        <td style="font-size: 18px; font-style: italic"><b><?= $open_balance ?></b></td>
                                                        <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_debit, 2, '.', '') ?> Rs</b></td>
                                                        <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_credit, 2, '.', '') ?> Rs</b></td>
                                                        <td style="font-size: 18px; font-style: italic"><b><?= number_format(($balance), 2, '.', '') ?> Rs</b></td>
                                                    </tr>
                                                    <?php
                                                } else {
...
//                                                        print_r($account_report[$i]);
//                                                        die();
                                                        $total_debit = 0;
                                                        $total_credit = 0;
                                                        $balance = $open_balance[$i];
                                                        echo "</tr>";


                                                        if (!empty($account_report[$i])) {
                                                            $ledger_count = 0;

                                                            foreach ($account_report[$i] as $value) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><?= $open_balance[$i] ?></td>
                                                            <td><?= number_format($total_debit, 2, '.', '') ?> Rs</td>
                                                            <td><?= number_format($total_credit, 2, '.', '') ?> Rs</td>
                                                            <td><?= number_format(($balance), 2, '.', '') ?> Rs</td>
                                                        </tr>

                                                        <?php
                                                        $total_credit1 = $total_credit1 + $total_credit;
                                                        $total_debit1 = $total_debit1 + $total_debit; 
                                                ...
                                                <td></td>
                                                <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_debit1, 2, '.', '') ?> Rs</b></td>
                                                <td style="font-size: 18px; font-style: italic"><b><?= number_format($total_credit1, 2, '.', '') ?> Rs</b></td>
                                                <td style="font-size: 18px; font-style: italic"><b><?= number_format(($total_balance), 2, '.', '') ?> Rs</b></td>
                                                </tr>
                                                    <?php
                                                }
                                                ?>
            ...
            <script>
                                    ...
                                    function printAccountLedgerProduct() {
                                        $(".my_hidden").addClass("hidden-print");
                                        printAccountLedger('PrintArea');
